...
const ids = ['space','flat','surface-select','equation','apply','equation-error','equation-hint','slice-select','c-range','a-range','b-range','c-row','a-row','b-row','c-label','c-value','a-value','b-value','reset-view','flat-title','flat-caption','flat-metric','surface-set','plane-set','intersection-set','flat-set','explanation'];
...
assert.match(nodes['intersection-set'].textContent,/I = S ∩ P = \{\(x, y, 2\)/);
assert.match(nodes['intersection-set'].textContent,/x² \+ y² = 2/);
assert.match(nodes['flat-set'].textContent,/C = \{\(x, y\) ∈ ℝ²/);
...
nodes['c-range'].value=-.5;nodes['c-range'].listeners.input({target:nodes['c-range']});
assert.equal(nodes['intersection-set'].textContent,'I = S ∩ P = ∅');
assert.equal(nodes['flat-set'].textContent,'C = ∅');
...
assert.match(nodes['intersection-set'].textContent,/\{\(x, 0, x² \+ 0\)/);
assert.match(nodes['flat-set'].textContent,/\{\(x, x² \+ 0\)/);
...
assert.match(nodes['intersection-set'].textContent,/\{\(0, y, 0 \+ y²\)/);
...
assert.match(nodes['intersection-set'].textContent,/\(x, y, x² \+ y²\).*0.8x \+ 0.4y \+ 1.5/);
...
near(state.f(0,0),1);
assert.match(nodes['intersection-set'].textContent,/sin\(x\) \+ cos\(y\)/);
nodes.equation.value='z = x² + y²';nodes.apply.listeners.click();
assert.equal(state.expr,'x² + y²');near(state.f(1,2),5);
nodes.equation.value='alert(1)';nodes.apply.listeners.click();
assert.equal(nodes['equation-error'].hidden,false);
assert.equal(state.expr,'x² + y²');
